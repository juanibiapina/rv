...
impl<'a> App<'a> {
    ...
    fn handle_commit_list_key(&mut self, key: KeyCode) -> Action {
        match key {
            ...
            KeyCode::PageDown => {
                self.commit_scroll.page_down(self.commits.len());
                self.check_commit_selection_changed()
            }
            KeyCode::PageUp => {
                self.commit_scroll.page_up();
                self.check_commit_selection_changed()
            }
            _ => Action::None,
        }
...
mod tests {
    ...
    // --- Enter drill-in tests ---

    #[test]
    fn commit_list_enter_switches_to_file_list() {
        let mut app = App::new(sample_commits());
        app.set_commit_files(sample_files());
        let action = app.handle_key(KeyCode::Enter);
        assert_eq!(action, Action::Render);
    ...
    #[test]
    fn commit_list_enter_stays_when_no_files() {
        let mut app = App::new(sample_commits());
        let action = app.handle_key(KeyCode::Enter);
        assert_eq!(action, Action::Render);
        assert_eq!(app.active_panel, Panel::CommitList);
    ...
    #[test]
    fn file_list_backtab_goes_to_commit_list() {
        let mut app = app_with_files();
        app.active_panel = Panel::FileList;
    ...
    // --- J/K cross-panel diff scroll tests ---

    #[test]
    fn commit_list_shift_j_scrolls_diff() {
        let mut app = App::new(sample_commits());
        app.set_commit_files(sample_files());
        app.set_diff_content(Text::raw("line1\nline2\nline3"), 3);
    ...
    #[test]
    fn commit_list_shift_k_scrolls_diff() {
        let mut app = App::new(sample_commits());
        app.set_commit_files(sample_files());
        app.set_diff_content(Text::raw("line1\nline2\nline3"), 3);
        app.diff_scroll.visible_rows = 10;
        app.diff_scroll.cursor = 2;
        let action = app.handle_key(KeyCode::Char('K'));
        assert_eq!(action, Action::Render);
        assert_eq!(app.diff_scroll.cursor, 1);
        assert_eq!(app.active_panel, Panel::CommitList);
    ...
    // --- PgUp/PgDn in commit and file lists ---

    #[test]
    fn commit_list_page_down() {
        let mut app = App::new(sample_commits());
        app.commit_scroll.visible_rows = 2;
